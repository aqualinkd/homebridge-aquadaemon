      ...
      for (var i = 0; i < rawDevices.length; i++) {
        ...
        if (device.name != "NONE" && device.label != "NONE" && device.id != "") {
          ...
          if (mappedType !== null) {
              ...
              device.hkType = Utils.adDevice2hkSDevice(this.platform.instanceName, mappedType);
              if (server === Constants.serverType.AQUALINKD) {
                if (this.platform.useLegacyTempSensors == false) {
                ...
                if (this.platform.config.use_tank_volume_uom && device.mappedType === Constants.AdDeviceType.LEVEL_SENSOR) {
                  this.platform.log("Using tank volume UOM for " + device.name + " (" + device.id + ")");
                  device.id = device.id.endsWith("/level/remaining") ? device.id : device.id + "/level/remaining"; // Add /level/remaining so uses UOM from AquachemD instead of percentage.
                }
              }
