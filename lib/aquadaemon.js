...
Aquadaemon.prototype.updateDeviceStatus = function(accessory, value, callback, statustype = Constants.statusStatus) {
  ...
  this.platform.forceLog("updateDeviceStatus: "+accessory.id+" value="+value+" statustype="+statustype);
  ...
  if (accessory.platform.mqtt) {
    ...
    if (accessory.platform.connectedServerType == Constants.serverType.AQUACHEMD && accessory.mappedType === Constants.AdDeviceType.DOSER ) {
      // For Doser we will send off if it's actually on, and enable otherwise.
      //if (value === 0 && accessory.fullState === 1) {
     // }
