...
function AquadaemonAccessory(platform, platformAccessory, id, device, uuid) {
  ...
  this.hkType = device.hkType || Utils.adDevice2hkSDevice(this.platform.instanceName, this.mappedType);
...
AquadaemonAccessory.prototype = {
  ...
  setState : function(state, callback, context) {
    ...
    // Valve & Sprinkler will send two off messages (Active & InUse), this will confuse AquachemD that has on/off/enable, as the
    // 2nd off will turn it off vs enabled, so ignore the 2nd message
    if ( state == 0 && this.state == 0 &&
         this.platform.connectedServerType === Constants.serverType.AQUACHEMD &&
         (this.hkType === Constants.hkDeviceType.VALVE || this.hkType === Constants.hkDeviceType.SPRINKLER)) {
      callback(); return;
    }
  ...
  getSensorValue : function(callback) {
    ...
    // Not sure below is needed or accurate, come back and check
    ...
    if (this.hkType === Constants.hkDeviceType.LUX_SENSOR && val < 0.0001) val = 0.0001;
    if (this.hkType === Constants.AdDeviceType.TEMPERATURE_SENSOR && typeof this.value === 'undefined') {
       val = Utils.parseAccessoryValue(this.mappedType, 0);
    }
  ...
  getServices : function() {
    ...
    //} else if (this.mappedType === Constants.AdDeviceType.TEMPERATURE_SENSOR ) {
      ...
      if (this.mappedType !== Constants.AdDeviceType.TEMPERATURE_SENSOR) {
        this.getCharacteristic(service, Characteristic.CurrentTemperature).setProps({format: "float", minValue: Constants.adValueMin, maxValue: Constants.adValueMax});
      } else {
        this.getCharacteristic(service, Characteristic.CurrentTemperature).setProps({minValue: Constants.adTempMin, maxValue: Constants.adTempMax});
      } 
    //} else if (this.mappedType === Constants.AdDeviceType.LUX_SENSOR || this.mappedType === Constants.AdDeviceType.PH_SENSOR || this.mappedType === Constants.AdDeviceType.ORP_SENSOR) {
      ...
      if (this.mappedType === Constants.AdDeviceType.PH_SENSOR ) {
        ...
        // Use standard float value limits for chemistry readings (% 0-100) Same values are valid for UOM of gallons or litres, as reported by AquachemD
    ...
    //} else if (this.mappedType === Constants.AdDeviceType.HEATER_THERMOSTAT || this.mappedType === Constants.AdDeviceType.SWG_CONTROLLER || this.mappedType === Constants.AdDeviceType.FREEZE_PROTECT || this.mappedType === Constants.AdDeviceType.CHILLER_THERMOSTAT) {
      ...
      var niceSteps = [1, 5, 10, 15, 30, 60, 120, 300, 600, 900, 1800, 3600];
      ...
      var minStep = 0; // Default fallback

      var idealStep = maxValue / 10; // Aim for 10 steps, but will round up to nearest nice step

      for (var i = 0; i < niceSteps.length; i++) {
        if (niceSteps[i] >= idealStep) {
          minStep = niceSteps[i];
          break;
        }
      }
      if (minStep === 0) minStep = niceSteps[niceSteps.length - 1]; // Fallback to largest step if none found
      ...
      this.getCharacteristic(service, Characteristic.RemainingDuration).setProps({minValue: 0, maxValue: maxValue, minStep: 1});
  ...
  /*
    Example topics to be processed.
    aquachemd/CS_1/state
    aquachemd/PMP_1/timer/state
    aquachemd/PMP_1/timer/duration
    aquachemd/PMP_1/timer/default
    aquachemd/PMP_1
    aqualinkd/SWG/enabled
    aqualinkd/SWG/setpoint
    aqualinkd/SWG/Percent_f
    aqualinkd/Pool_Heater/setpoint
    aqualinkd/Filter_Pump/Speed
    aqualinkd/Aux_3/brightness
    aqualinkd/Temperature/Pool
  */


  handleMQTTMessage: function (topic, message, callback) {
    if (this._handleStateTopic(topic, message, callback)) return;
    if (this._handleValueTopic(topic, message, callback)) return;
    if (this._handleSubtopic(topic, message, callback)) return;
    this._handleLegacyTopic(topic, message, callback);
  },

  _getChar: function(serviceType, characteristicType) {
  ...
  _handleStateTopic: function (topic, message, callback) {
    if (topic !== this.id + "/state") return false;
    ...
    if (this.hkType === Constants.hkDeviceType.CONTACT_SENSOR) {
      ...
      callback(this._getChar(Service.ContactSensor, Characteristic.ContactSensorState),
        value === 1 ? Characteristic.ContactSensorState.CONTACT_DETECTED : Characteristic.ContactSensorState.CONTACT_NOT_DETECTED);
      return true;
    }
    if (this.hkType === Constants.hkDeviceType.SWITCH) {
      this.fullState = value;
      callback(this._getChar(Service.Switch, Characteristic.On), Utils.parseAccessoryState(this.mappedType, value));
      return true;
    }
    if (this.hkType === Constants.hkDeviceType.VALVE || this.hkType === Constants.hkDeviceType.SPRINKLER) {
      this.fullState = value;
      var parsed = Utils.parseAccessoryState(this.mappedType, value);
      //this.platform.forceLog("Valve " + this.id + " state received: " + message);
      //this.platform.forceLog("SetState "+this.id+" value "+parsed+" + fromMessage="+value);
      // Update internal state so get handlers return the correct value
      //this.state = parsed; 
      callback(this._getChar(Service.Valve, Characteristic.Active), parsed);
      callback(this._getChar(Service.Valve, Characteristic.InUse), parsed);
      /*
      if (parsed === 1) {
        // When turning ON via MQTT, push the duration so the countdown appears
        var duration = this.timerRemainingDuration > 0 ? this.timerRemainingDuration : this.timerDefaultDuration;
        if (duration > 0) {
          callback(this._getChar(Service.Valve, Characteristic.RemainingDuration), duration);
        }
      } else {
        // When turning OFF, reset the timer
        callback(this._getChar(Service.Valve, Characteristic.RemainingDuration), 0);
      }*/
      return true;
    }
    return false;
  ...
  _handleValueTopic: function (topic, message, callback) {
    if (topic !== this.id) return false;

    switch (this.hkType) {
      case Constants.hkDeviceType.SWITCH:
        callback(this._getChar(Service.Switch, Characteristic.On), parseInt(message));
        return true;
      case Constants.hkDeviceType.FAN:
        callback(this._getChar(Service.Fan, Characteristic.On), parseInt(message));
        return true;
      case Constants.hkDeviceType.DIMMER:
        callback(this._getChar(Service.Lightbulb, Characteristic.On), parseInt(message));
        return true;
      case Constants.hkDeviceType.TEMPERATURE_SENSOR:
        this.value = this._resolveTemperature(message);
        callback(this._getChar(Service.TemperatureSensor, Characteristic.CurrentTemperature), this.value);
        return true;
      case Constants.hkDeviceType.LUX_SENSOR:
        ...
        callback(this._getChar(Service.LightSensor, Characteristic.CurrentAmbientLightLevel), this.value);
        return true;
      case Constants.hkDeviceType.CO2_SENSOR:
        this.value = parseFloat(message);
        if (isNaN(this.value)) this.value = 0;
        callback(this._getChar(Service.CarbonDioxideSensor, Characteristic.CarbonDioxideLevel), this.value);
        return true;
      case Constants.hkDeviceType.THERMOSTAT:
        callback(this._getChar(Service.Thermostat, Characteristic.CurrentHeatingCoolingState), parseInt(message));
        return true;
      default:
        return false;
    }
  ...
  _handleSubtopic: function (topic, message, callback) {
    var idPos = topic.indexOf("/");
    if (idPos === -1 || this.id !== topic.substring(0, idPos)) return false;

    var subtopic = topic.substring(idPos + 1);

    switch (subtopic) {
      case "enabled": {
        var v = parseInt(message);
        if (this.mappedType === Constants.AdDeviceType.FREEZE_PROTECT && v === 1) v = 2; // Fixes valid-values-array warning
        callback(this._getChar(Service.Thermostat, Characteristic.TargetHeatingCoolingState), v);
        return true;
      }
      case "setpoint":
        callback(this._getChar(Service.Thermostat, Characteristic.TargetTemperature), Utils.parseAccessoryTargetValue(this.mappedType, message));
        return true;
      case "Speed":
        if (this.hkType !== Constants.hkDeviceType.FAN) return false;
        callback(this._getChar(Service.Fan, Characteristic.RotationSpeed), Utils.parseAccessoryTargetValue(this.mappedType, message));
        return true;
      case "brightness":
        if (this.hkType !== Constants.hkDeviceType.DIMMER) return false;
        callback(this._getChar(Service.Lightbulb, Characteristic.Brightness), Utils.parseAccessoryTargetValue(this.mappedType, message));
        return true;
      case "timer/default":
        if (this.hkType !== Constants.hkDeviceType.VALVE) return false; 
        //this.platform.forceLog("Valve " + this.id + " timer/default received: " + message);
        //this.timerDefaultDuration = Utils.parseAccessoryTargetValue(this.mappedType, message);
        callback(this._getChar(Service.Valve, Characteristic.SetDuration), Utils.parseAccessoryTargetValue(this.mappedType, message));
        return true;
      case "timer/duration":
        if (this.hkType !== Constants.hkDeviceType.VALVE) return false;
        //this.platform.forceLog("Valve " + this.id + " timer/duration received: " + message);
        //this.timerRemainingDuration = Utils.parseAccessoryTargetValue(this.mappedType, message);
        callback(this._getChar(Service.Valve, Characteristic.RemainingDuration), Utils.parseAccessoryTargetValue(this.mappedType, message));
        return true;
      case "timer/state":
        if (this.hkType !== Constants.hkDeviceType.VALVE) return false;
        //this.platform.forceLog("Valve " + this.id + " timer/state received: " + message);  
        callback(this._getChar(Service.Valve, Characteristic.InUse), Utils.parseAccessoryState(this.mappedType, parseInt(message)));
        return true;
      default:
        return false;
    }
  ...
  _handleLegacyTopic: function (topic, message, callback) {
    if (topic === "Temperature/Pool" && (this.id === "Pool_Heater" || this.id === "Chiller")) {
      callback(this._getChar(Service.Thermostat, Characteristic.CurrentTemperature), Utils.parseAccessoryValue(this.mappedType, message));
      return true;
    }
    if (topic === "Temperature/Spa" && this.id === "Spa_Heater") {
      callback(this._getChar(Service.Thermostat, Characteristic.CurrentTemperature), Utils.parseAccessoryValue(this.mappedType, message));
      return true;
    }
    if (topic === "Temperature/Air" && this.id === "Freeze_Protect") {
      callback(this._getChar(Service.Thermostat, Characteristic.CurrentTemperature), Utils.parseAccessoryValue(this.mappedType, message));
      return true;
    }
    if (topic === "SWG/Percent_f" && this.id === "SWG") {
      callback(this._getChar(Service.Thermostat, Characteristic.CurrentTemperature), Utils.parseAccessoryValue(this.mappedType, message));
      return true;
    }
    return false;
  ...
  _resolveTemperature: function (message) {
    var isLegacyNonTemp = this.platform.useLegacyTempSensors === true &&
      this.platform.connectedServerType === Constants.serverType.AQUALINKD &&
      this.mappedType !== Constants.AdDeviceType.TEMPERATURE_SENSOR &&
      this.id.slice(-2) !== "_f" &&
      this.sourceUOM !== "°C";

    if (isLegacyNonTemp) {
      var value = Utils.degFtoC(Utils.parseAccessoryValue(this.mappedType, message));
    ...
    var value = Utils.parseAccessoryValue(this.mappedType, message);
  ...
  /**********************************
   * 
   * OLD Code to handle MQTT messages
   * Only here while debugging to make sure new code caught everything.
  ...
  handleMQTTMessage : function(topic, message, callback) {
    ...
    if (this.id+"/state" == topic) {
      ...
      } else if (this.hkType === Constants.hkDeviceType.SWITCH) {
      ...
      } else if (this.hkType === Constants.hkDeviceType.VALVE || this.hkType === Constants.hkDeviceType.SPRINKLER) {
        //this.setFullState((value === 1)?1:0, function() {}, "Aquadaemon-MQTT");
        this.fullState = value;
        service = this.getService(Service.Valve);
        //characteristic = this.getCharacteristic(service, Characteristic.InUse);
        value = Utils.parseAccessoryState(this.mappedType, value);
        characteristic = this.getCharacteristic(service, Characteristic.Active);
        // Have to make 2 calls, Active and inUse, so call one here and let the second fall through as necessary
        callback(characteristic, value); 
        characteristic = this.getCharacteristic(service, Characteristic.InUse);
        // The above callback will get called at end of function.
      }
    }

    // Value or Status of any device where it's just a topic of ID.
    // So state of AqualinkD device, Value of AqualinkD or AquachemD
    if (this.id == topic) {
      if (this.hkType === Constants.hkDeviceType.SWITCH ) {
        service = this.getService(Service.Switch);
        characteristic = this.getCharacteristic(service, Characteristic.On);
        ...
        if (this.platform.useLegacyTempSensors === true &&
            this.platform.connectedServerType === Constants.serverType.AQUALINKD && // Should be redundant with above test
            //this.type === Constants.adDeviceValue && 
            this.mappedType !== Constants.AdDeviceType.TEMPERATURE_SENSOR &&
            this.id.slice(-2) !== "_f" &&
            this.sourceUOM !== "°C") 
        {
