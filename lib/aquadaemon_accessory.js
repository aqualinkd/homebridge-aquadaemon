  ...
  handleMQTTMessage : function(topic, message, callback) {
    ...
    if (this.id == topic) {
      ...
      //} else if (this.mappedType === Constants.AdDeviceType.TEMPERATURE_SENSOR ) {
        ...
        /*
        this.platform.forceLog(
            `Topic=${topic}` +
            `,Legacy=${this.platform.useLegacyTempSensors ? "yes" : "no"}` +
            `,Server=${this.platform.connectedServerType === Constants.serverType.AQUALINKD ? "aqualink" : "aquachem"}` +
            `,Mapped=${Utils.adDevice2String(this.mappedType)}` +
            `,topic=${this.id.slice(-2)}` +
            `,UOM=${this.sourceUOM}`
        );
        */
