     ...
     switch (device.type) {
       ...
       case Constants.adDeviceSwitchPrg:
        if (device.type_ext == Constants.adDeviceSwitchVSP) {
          ...
          mappedType = Constants.AdDeviceType.SWITCH_PROGRAM; break;
...
/**
 * Register a custom mapping from an AdDeviceType to a hkDeviceType.
 * Can be called multiple times to register multiple custom overrides.
 */
Utils.addCustomadDevice2hkSDeviceMap = function(adDevice, hkDevice) {
  if (adDevice !== undefined && hkDevice !== undefined) {
    _customMappings[adDevice] = hkDevice;
  }
}
 ...
 *****************************************************************************/
// Currently only work for AquachemD and NOT AqualinkD
Utils.adDevice2hkSDevice = function(adDevice) {

  // Check custom overrides first
  if (_customMappings.hasOwnProperty(adDevice)) {
    return _customMappings[adDevice];
  }
